  ...
  card.innerHTML = `
    <div class="project__top">
      ...
      <div class="badges" aria-label="Project metadata">
        ${badges.map((b) => `<span class="badge">${escapeText(b)}</span>`).join('')}
      </div>
    </div>
...
function renderLanguageRatings() {
  ...
  for (const language of languages) {
    ...
    if (fillEl instanceof HTMLElement) {
      fillEl.dataset.fill = String(fillPercent);
      fillEl.style.width = prefersReducedMotion() ? `${fillPercent}%` : '0%';
    }
